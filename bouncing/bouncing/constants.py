from ament_index_python.packages import get_package_share_directory
from pathlib import Path

from nectar.vision import OpenCVConfig


CAMERA_IMAGE_SOURCE = 'webcam'
CAMERA_CONFIG = OpenCVConfig(
    width=1280,
    height=720,
    threaded=False,
    device_index=0,
)

MODEL_SOURCE = str(
    Path(get_package_share_directory("bouncing")) / "models" / "best.pt"
)
MODEL_CONFIDENCE_THRESHOLD = 0.5

TAKEOFF_ALTITUDE = 5.0  # m

SEARCH_NUMBER_DETECTIONS = 100
SEARCH_DETECTIONS_LOST_TOLERANCE = 10

RECOVERY_STEP = 0.5  # m
RECOVERY_LIMITE_ALTITUDE = 7.0  # m

PRECISE_LIMITE_ALTITUDE = 7.0  # m
PRECISE_HOVER_COUNT = 15  # number of consecutive detections to consider the landing successful
PRECISE_RESET_PID = 2 #  number of detections lost to reset the PID controller
PRECISE_LOST_TOLERANCE = 15
PRECISE_TIMEOUT = 3 * 60  # s
PRECISE_VERTICAL_SPEED = 0.5  # m/s
PRECISE_ALING_TOLERANCE = 50  # pixel
PRECISE_LAND_ALTITUDE = 1.0  # m

HOVER_DETECTIONS_LOST_RESET_PID_TOLERANCE = 2
HOVER_DETECTIONS_LOST_TOLERANCE = 15
HOVER_TIMEOUT = 60
HOVER_ALING_TOLERANCE = 30
HOVER_ALING_COUNT_TO_SUCCEED = 15

CONTROLER_P_XY = 0.0013457
CONTROLER_I_XY = 0.0001
CONTROLER_D_XY = 0.0
CONTROLER_OUTPUT_LIMITS_XY = (-0.3, 0.3)
CONTROLER_INTEGRAL_LIMITS_XY = (0.1, -0.1)
