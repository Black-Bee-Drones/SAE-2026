...
def draw_hud(
    ...
    *,
    ...
    if not lines:
        return
    sizes = [cv2.getTextSize(line, _FONT, text_scale, text_thickness)[0] for line in lines]
    ...
    for i, line in enumerate(lines):
        ...
        cv2.putText(
            img, line, (x0 + pad, y),
            _FONT, text_scale, COLOR_HUD_TEXT, text_thickness, cv2.LINE_AA,
        )
...
def _draw_target_hose_row(
    ...
    if tol_px > 1:
        ...
        cv2.line(img, (0, upper), (image_width, upper), COLOR_TARGET_HOSE, 1, cv2.LINE_AA)
        cv2.line(img, (0, lower), (image_width, lower), COLOR_TARGET_HOSE, 1, cv2.LINE_AA)
...
def _draw_error_arrows(
    ...
    if sphere_xy is not None:
        cv2.arrowedLine(
            ...
            COLOR_ERR_VY, 2, tipLength=0.15, line_type=cv2.LINE_AA,
        )
    ...
    cv2.arrowedLine(
        ...
        COLOR_ERR_VX, 2, tipLength=0.18, line_type=cv2.LINE_AA,
    )
...
def _draw_altitude_bar(
    ...
    draw_dashed_line(img, (x0 - 8, yr), (x1 + 8, yr), COLOR_ALT_BAR_RELEASE, thickness=2, dash=8, gap=4)
    cv2.putText(
        img, f"R={release_alt:.1f}",
        (x0 - 80, yr + 5), _FONT, 0.5, COLOR_ALT_BAR_RELEASE, 1, cv2.LINE_AA,
    )
    ...
    if altitude is not None:
        ...
        cv2.putText(
            img, f"{altitude:.2f}m",
            (x0 - 90, ya - 4), _FONT, 0.5, COLOR_ALT_BAR, 1, cv2.LINE_AA,
        )
...
def draw_align(
    ...
    *,
    ...
    """ALIGN composite overlay (modifies ``img`` in place)."""
    H, W = img.shape[:2]
    ...
    _draw_target_sphere(img, target_sphere_xy, tol_anchor_px, in_fov=True)
    ...
    if hose_pose is not None:
        hose_cy = hose_pose[1]
        _draw_error_arrows(
            img, sphere_center, target_sphere_xy, hose_cy, target_hose_cy, hook_xy
        )
    ...
    draw_hud(
        img,
        [
            f"ALIGN[{phase:>4s}]  alt={alt_txt}  ppm={ppm:5.0f}",
...
def draw_descend(
    ...
    *,
    ...
    """DESCEND composite overlay (modifies ``img`` in place)."""
    H, W = img.shape[:2]
    tol_center_px = max(tol_center_m * ppm, 1.0)
    tol_anchor_px = max(tol_anchor_m * ppm, 1.0)
    ...
    alt_txt = f"{altitude:.2f}m" if altitude is not None else "n/a"
    anchor_phase = "full" if sphere_can_anchor else "hose-only"
    draw_hud(
        img,
        [
            f"DESCEND  alt={alt_txt}  ppm={ppm:5.0f}",
            f"phase: anchor={anchor_phase}",
            f"err: hose_cy={err_center_m:+.3f}m ({err_center_px:+5.0f}px)",
            f"     anchor  ={err_anchor_m:+.3f}m ({err_anchor_px:+5.0f}px)",
            f"     angle   ={err_angle_deg:+5.1f}deg",
...
def draw_orient(
    ...
    *,
    ...
    """ORIENT_TO_HOOK composite overlay.

    Draws the image center, the live sphere centroid, the chosen rope axis
    (when available), a dashed yellow ray to ``theta_target`` (where the
    sphere should appear after the 180° spin), a magenta ray to
    ``theta_current``, and HUDs with the sample/spin status and commanded
    ``vyaw``. Modifies ``img`` in place.
    """
    H, W = img.shape[:2]
    ...
    if theta_target is not None:
        ...
        draw_dashed_line(
            img, (cx_img, cy_img), (tx, ty),
            COLOR_TARGET_SPHERE, thickness=2, dash=18, gap=10,
        )
        cv2.drawMarker(
            img, (tx, ty), COLOR_TARGET_SPHERE,
    ...
    if theta_current is not None:
        ...
        cv2.line(
            img, (cx_img, cy_img), (cx_ray, cy_ray),
            COLOR_HOOK, 2, cv2.LINE_AA,
        )
    ...
    rope_str = (
        f"rope body_x={rope_body_x:+.3f}m  thr=-{behind_threshold_m:.2f}m"
        if rope_body_x is not None
        else "rope body_x=  n/a"
    )
    ...
    draw_hud(
        img,
        [
            f"ORIENT_TO_HOOK[{phase}]  {sample_str}",
            rope_str,
            theta_curr_str,
            theta_tgt_str,
            err_str,
    ...
    *,
    ...
    if sphere_center is not None:
        ...
        if side_unit is not None:
            ...
            cv2.arrowedLine(img, (sx, sy), tip, color, 4, tipLength=0.15, line_type=cv2.LINE_AA)
